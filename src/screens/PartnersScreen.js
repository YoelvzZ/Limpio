...
export default function PartnersScreen({ navigation }) {
  ...
  const handleSavePartner = async () => {
    if (!formData.name || !formData.percentage || !formData.partnerUserId) {
      Alert.alert("Error", "Completa nombre, porcentaje y código de socio");
      return;
    }
    ...
    try {
      const partnerData = {
        ...
        partnerUserId: formData.partnerUserId || null,
      };
      ...
      if (editingPartner) {
        ...
        Alert.alert(
          ...
          "El socio debe aceptar para activar la conexión",
        );
      }
      ...
      Alert.alert("Error", error.message || "No se pudo guardar el socio");
    }
  };
  ...
  const handleShowStats = async (partner) => {
    ...
    try {
      ...
      const partnerEarnings = Math.floor(
        stats.totalRevenue * (partner.percentage / 100),
      );
  ...
  const handleAccept = async (partner) => {
    try {
      await acceptPartnerInvitation(partner.id);
      Alert.alert(
        "Conexión activa ✅",
        `Ahora puedes usar lavadoras de ${partner.name}`,
      );
      loadPartners();
    } catch (error) {
  ...
  const handleReject = async (partner) => {
    try {
      await rejectPartnerInvitation(partner.id);
      loadPartners();
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View>
          <Text style={styles.headerTitle}>Socios 👥</Text>
          <Text style={styles.headerSubtitle}>{partners.length} conexiones</Text>
        </View>
        <TouchableOpacity
          ...
          partners.map((partner) => (
            <View key={partner.id} style={styles.partnerCard}>
              ...
              <Text style={styles.statusText}>
                {partner.status === "active" ? "🟢 Activo" : "🟡 Pendiente"}
              </Text>

              {partner.status === "pending" &&
              partner.relationshipType === "incoming" ? (
                <View style={styles.partnerActions}>
                  <TouchableOpacity
                    style={[styles.actionButton, styles.statsButton]}
                    onPress={() => handleAccept(partner)}
                  >
                    <Text style={styles.actionButtonText}>✅ Aceptar</Text>
                  </TouchableOpacity>

                  <TouchableOpacity
                    style={[styles.actionButton, styles.deleteButtonInline]}
                    onPress={() => handleReject(partner)}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      setFormData({
                        name: partner.name || "",
                        phone: partner.phone || "",
                        percentage: (partner.percentage || 30).toString(),
                        notes: partner.notes || "",
          ...
          <View style={styles.modalContent}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <Text style={styles.hint}>
                Ejemplo: Si es 30%, el socio recibe $4,500 de un alquiler de
                $15,000
              </Text>
              ...
              <Text style={styles.label}>Código de Usuario (Firebase UID) *</Text>
              <TextInput
                style={styles.input}
                value={formData.partnerUserId}
                onChangeText={(text) =>
                  setFormData({ ...formData, partnerUserId: text })
                }
              ...
              />
              <Text style={styles.hint}>
                💡 Usa el código del perfil del socio. Cuando acepte, quedarán
                conectados y podrán compartir lavadoras.
              </Text>
                ...
                <View style={styles.statsHeader}>
                  <View style={styles.statsAvatar}>
                    <Text style={styles.statsAvatarText}>
                      {showStats.name?.charAt(0)?.toUpperCase() || "S"}
                    </Text>
                  </View>
                  ...
                  <>
                    <View style={styles.statsGrid}>
                      ...
                      <View style={[styles.statCard, { backgroundColor: "#F59E0B" }]}>
                        <Text style={styles.statCardIcon}>💵</Text>
                        <Text style={styles.statCardNumber}>
                          ${(statsData.partnerEarnings / 1000).toFixed(0)}k
                        </Text>
                      ...
                      <View style={[styles.statCard, { backgroundColor: "#10B981" }]}>
                        <Text style={styles.statCardIcon}>💎</Text>
                        <Text style={styles.statCardNumber}>
                          ${(statsData.yourEarnings / 1000).toFixed(0)}k
                        </Text>
                    ...
                    {statsData.totalRevenue > 0 && (
                      <View style={styles.calculationBox}>
                        ...
                        <Text style={styles.calculationText}>
                          • Socio ({showStats.percentage}%): $
                          {statsData.partnerEarnings.toLocaleString()}
                        </Text>
                        <Text style={styles.calculationText}>
                          • Tú ({100 - showStats.percentage}%): $
                          {statsData.yourEarnings.toLocaleString()}
                        </Text>
                      </View>
                    )}
